# row0: 新增圖片與動作按鈕、圖庫按鈕、Script按鈕、存檔按鈕、腳本下拉選單
# row1: 動作列表 Listbox
# row2: 執行按鈕
# row3: Listbox 綁定雙擊編輯延遲
# row4: Listbox 綁定拖曱排序（on_drag_start, on_drag_motion, on_drag_drop）
# row5: 儲存所有圖片於 images 資料夾
# row6: 儲存所有動作於 self.actions
# row7: 新增圖片時自動命名並複製到 images
# row8: 動作輸入視窗，支援鍵盤與滑鼠動作
# row9: 執行時按鈕禁用，結束後恢復
# row10: 找不到圖片時自動停止動作
# pyinstaller --noconsole --onedir ChroLens_Sothoth1.0.py
# 目前不會點擊圖片、按鍵會一直變成alt+按鍵
...
def normalize_hotkey(keystr):
    keystr = keystr.strip().replace(" ", "")
    if not keystr:
        return ""
    if keystr.startswith("<") and keystr.endswith(">"):
        return keystr
    parts = keystr.replace("+", "-").split("-")
    tkmap = {"ctrl": "Control", "control": "Control", "alt": "Alt", "shift": "Shift", "win": "Win", "windows": "Win"}
    parts = [tkmap.get(p.lower(), p.capitalize() if len(p)==1 else p.title()) for p in parts]
...
class ChroLens_SothothApp(tb.Window):
    def __init__(self):
        ...
        self.btn_hotkey = tb.Button(frm_top, text="快捷鍵", width=10, bootstyle=SECONDARY, command=self.open_hotkey_settings)
    ...
    def edit_action(self, act, idx):
        action_win = tk.Toplevel(self)
        action_win.title("請輸入動作")
        tb.Label(action_win, text="請在下方框框內按下你要的鍵盤或滑鼠動作", style="My.TLabel").pack(pady=5)
        entry = tk.Entry(action_win, width=40, font=("Microsoft JhengHei", 14))
        entry.pack(padx=10, pady=10)
        entry.focus_set()
        action = {"value": None}
        def on_key(event):
            action["value"] = event.keysym
        ...
        entry.bind("<Button-1>", on_mouse)
        entry.bind("<Button-2>", on_mouse)
        entry.bind("<Button-3>", on_mouse)
        entry.bind("<MouseWheel>", on_mouse_wheel)
        def on_ok():
            if not action["value"]:
                messagebox.showerror("錯誤", "請輸入動作")
                return
            act.action = action["value"]
            action_win.destroy()
            self.update_tree()
            self.log(f"編輯動作：{act.pic_key} - {act.action}")
        tb.Button(action_win, text="確定", bootstyle=SUCCESS, command=on_ok).pack(pady=5)
        action_win.grab_set()
    ...
    def add_action(self):
        ...
        win.title("新增動作/圖片/Script")
        win.geometry("540x300")
        win.resizable(False, False)

        # 1. 圖片
        frm_img = tb.Frame(win)
        frm_img.pack(fill="x", padx=10, pady=(12, 0))
        tb.Label(frm_img, text="圖片", width=6, anchor="w").pack(side="left")
        img_var = tk.StringVar()
        img_entry = tb.Entry(frm_img, textvariable=img_var, width=15, font=("Microsoft JhengHei", 11))
        img_entry.pack(side="left", fill="x", expand=True)
        delay_img_var = tk.DoubleVar(value=0)
        tb.Entry(frm_img, textvariable=delay_img_var, width=6).pack(side="left", padx=(8, 0))
        # 點擊圖片欄位框觸發選擇
        def select_img(event=None):
            path = filedialog.askopenfilename(title="選擇圖片", filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.bmp")])
            if path:
                img_var.set(path)
        img_entry.bind("<Button-1>", select_img)

        # 2. 按鍵（只捕捉一個動作：單一鍵盤鍵或滑鼠鍵，且輸入框只顯示不可手動輸入）
        frm_key = tb.Frame(win)
        frm_key.pack(fill="x", padx=10, pady=(10, 0))
        tb.Label(frm_key, text="按鍵", width=6, anchor="w").pack(side="left")
        key_var = tk.StringVar()
        key_entry = tb.Entry(frm_key, textvariable=key_var, width=15, font=("Microsoft JhengHei", 11), state="readonly")
        key_entry.pack(side="left", fill="x", expand=True)
        delay_key_var = tk.DoubleVar(value=0)
        tb.Entry(frm_key, textvariable=delay_key_var, width=6).pack(side="left", padx=(8, 0))

        import keyboard as kb
        import threading
        import pyautogui

        # 只捕捉一個鍵盤鍵（含win/ctrl/alt/shift/單鍵）
        def on_focus_in(event):
            def listen_key():
                try:
                    # --- 新增：如果有選圖片，先移動滑鼠到圖片中央 ---
                    img_path = img_var.get().strip()
                    orig_pos = None
                    moved = False
                    if img_path and os.path.exists(img_path):
                        # 搜尋螢幕圖片
                        pos = locate_image_on_screen(img_path)
                        if pos:
        ...
        frm_script.pack(fill="x", padx=10, pady=(10, 0))
        ...
        script_combo = tb.Combobox(frm_script, textvariable=script_var, values=script_files, width=15, state="readonly")
        ...
        record_key = self.hotkey_config.get("record", "f10").upper()
        record_stop_key = self.hotkey_config.get("record_stop", "f9").upper()
        tb.Button(frm_record, text=f"錄製({record_key})", width=14, bootstyle=PRIMARY, command=lambda: self.start_record_script(script_var)).pack(side="left", padx=(0, 8))
        tb.Button(frm_record, text=f"停止({record_stop_key})", width=14, bootstyle=WARNING, command=self.stop_record_script).pack(side="left")
        win.bind_all(f"<{record_key}>", lambda e: self.start_record_script(script_var))
        win.bind_all(f"<{record_stop_key}>", lambda e: self.stop_record_script())
        def on_close():
            win.unbind_all(f"<{record_key}>")
            win.unbind_all(f"<{record_stop_key}>")
            win.destroy()
        ...
        def on_add():
            ...
            delay = delay_img_var.get() if img_path else delay_key_var.get()
            # 圖片+點擊
            if img_path and key_action in ("左鍵點擊", "右鍵點擊"):
                ...
                act = Action(pic_key, save_path, key_action, delay)
                self.actions.append(act)
                self.log(f"新增圖片點擊動作：{pic_key} {key_action} 延遲{delay}秒")
            else:
                # 單純圖片
                if img_path:
                    pic_num = len(self.pic_map) + 1
                    ...
                    self.actions.append(act)
                    self.log(f"新增圖片動作：{pic_key} 延遲{delay_img_var.get()}秒")
                # 單純按鍵
                if key_action and key_action not in ("左鍵點擊", "右鍵點擊"):
                    act = Action("", "", key_action, delay_key_var.get())
                    self.actions.append(act)
                    self.log(f"新增按鍵動作：{key_action} 延遲{delay_key_var.get()}秒")
            # Script
            if script_var.get():
                act = Action("", "", f"[SCRIPT]{script_var.get().strip()}", delay_script_var.get())
                self.actions.append(act)
                self.log(f"新增Script動作：{script_var.get().strip()} 延遲{delay_script_var.get()}秒")
            self.update_tree()
            win.destroy()
        tb.Button(win, text="新增", bootstyle=SUCCESS, width=12, command=on_add).pack(pady=12)
        win.grab_set()
    ...
    def open_script_merge(self):
        ...
        def on_delete():
            ...
            if not messagebox.askyesno("確認", f"確定要刪除選取的腳本？\n{', '.join(names)}"):
                return
            for name in names:
                path = os.path.join(SCRIPTS_DIR, name + ".json")
                try:
                    os.remove(path)
                    self.log(f"已刪除腳本：{name}")
                except Exception as e:
        ...
        if os.path.exists(config_path):
            try:
                ...
                for k, v in config.items():
                    config[k] = normalize_hotkey(v)
                return {**default_hotkeys, **config}
            except Exception:
    ...
    def register_hotkeys(self):
        # 先解除所有舊的綁定
        for func in ["執行", "停止", "新增動作", "Script", "圖庫"]:
            try:
                self.unbind_all(normalize_hotkey(self.hotkey_config[func]))
            except Exception:
                pass
        # 重新綁定
        self.bind_all(normalize_hotkey(self.hotkey_config["執行"]), lambda e: self.run_actions())
        self.bind_all(normalize_hotkey(self.hotkey_config["停止"]), lambda e: self.stop_actions())
        self.bind_all(normalize_hotkey(self.hotkey_config["新增動作"]), lambda e: self.add_action())
        self.bind_all(normalize_hotkey(self.hotkey_config["Script"]), lambda e: self.open_script_merge())
        self.bind_all(normalize_hotkey(self.hotkey_config["圖庫"]), lambda e: self.open_gallery())
    ...
    def start_record_script(self, script_var):
        ...
        def do_record():
            ...
            filtered_k_events = [
                e for e in k_events
                if not (e.name in ('f10', 'f9') and e.event_type in ('down', 'up'))
            ]
            events = [
